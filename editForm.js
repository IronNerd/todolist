...
function raisePriority() {
  //item number will change to a lower number (change to to a higher priority)
  // delete task we are about to duplicate
  tasksList.splice(originalItemNoOfTask - 1, 1);
  // determine insertion point
  for (
    let PointerToTaskOfInterest = 0; PointerToTaskOfInterest < tasksList.length; PointerToTaskOfInterest++
  ) {
    ...
    if (taskOfInterestItemNo == finalItemNoForTask) {
      ...
      for (
        let pointerToDuet = 0; pointerToDuet < tasksList.length; pointerToDuet++
      ) {
...
function saveText() {
  ...
  if (originalItemNoOfTask == finalItemNoForTask) {
    ...
    // Save object to storage
    storage();
  } else if (finalItemNoForTask < originalItemNoOfTask) {
    // finalItemNoForTask < originalItemNoOfTask ->
    ...
    // finalItemNoForTask > originalItemNoOfTask -> item number will change to a higher number (change to to a lower priority)
